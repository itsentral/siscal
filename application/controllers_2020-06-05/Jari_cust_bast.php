...
class Jari_cust_bast extends CI_Controller {	
	...
	public function index($Periode='') {
		...
		$WHERE			= "det_bast.datet = '$Periode'
						AND det_bast.`status` != 'CNC'";
		$Query			= "SELECT
							det_bast.id,
							det_bast.nomor,
							det_bast.datet,
							det_bast.kode AS nocust,
							det_bast.`name` AS customer,
							det_bast.address,
							det_bast.pic,
							(
								CASE
								WHEN det_bast.flag_type = 'SUPP'
								AND det_bast.type_bast = 'DEL' THEN
									'Kirim Ke Subcon'
								WHEN det_bast.flag_type = 'SUPP'
								AND det_bast.type_bast = 'REC' THEN
									'Ambil Dari Subcon'
								WHEN det_bast.flag_type = 'CUST'
								AND det_bast.type_bast = 'DEL' THEN
									'Kirim Ke Customer'
								ELSE
									'Ambil Dari Customer'
							...
							det_bast.spk_driver_id,
							det_bast.letter_order_id,
							det_bast.no_so,
							det_spk.member_id AS driver_id,
							det_spk.member_name AS driver
						FROM
							bast_headers det_bast
						INNER JOIN spk_drivers det_spk ON det_bast.spk_driver_id = det_spk.id
						WHERE ".$WHERE;
		//echo $Query;
		...
		$data			= array(
			'action'		=> 'index',
			'title'			=> 'BAST Customer Or Supplier',
			'rows_data'		=> $records,
			'periode'		=> $Periode,
			'akses_menu'	=> $Arr_Akses
		);
		$this->load->view($this->folder.'/list_bast_cust',$data);
		
	}
	
	function view_bast_cust($kode_bast=''){
		$Arr_Akses		= $this->Arr_Akses;
		if($Arr_Akses['read'] !='1'){
			$this->session->set_flashdata("alert_data", "<div class=\"alert alert-warning\" id=\"flash-message\">You Don't Have Right To Access This Page, Please Contact Your Administrator....</div>");
		...
		$rows_header	= $this->db->get_where('bast_headers',array('id'=>$kode_bast))->result();
		$rows_driver	= $this->db->get_where('spk_drivers',array('id'=>$rows_header[0]->spk_driver_id))->result();
		$data			= array(
			'action'		=> 'view_bast_cust',
			'title'			=> 'BAST Customer Or Supplier Detail',
			'rows_header'	=> $rows_header,
			'rows_detail'	=> $rows_detail,
			'rows_driver'	=> $rows_driver
		);
		$this->load->view($this->folder.'/detail_bast_cust',$data);
	}
	...
	function download_excel(){
		...
		$Nama_File		= "BAST ".$this->input->post('periode');
		// Set Creator & Title
		$style_header = array(
            'borders' => array(
                    'allborders' => array(
		...
		$intD			= 0;
		$Query_Header	= "SELECT * FROM bast_headers WHERE id IN ('".$rows_Pilih."')";
		...
		if($rows_Header){
			...
			foreach($rows_Header as $keyH=>$valH){
				...
				$tgl_Bast		= $valH->datet;
				$cust_Bast		= $valH->name;
				$nocust_Bast	= $valH->kode;
				$jenis_Bast		= $valH->flag_type;
				$cat_Bast		= $valH->type_bast;
				$noso_Bast		= $valH->no_so;
				$letter_Bast	= $valH->letter_order_id;
				$spk_Bast		= $valH->spk_driver_id;
				...
				$rows_detail	= $this->db->get_where('bast_details',array('bast_header_id'=>$kode_Bast))->result();
				$rows_driver	= $this->db->get_where('spk_drivers',array('id'=>$spk_Bast))->result();
				
				$Driver_Name	= $rows_driver[0]->member_name;
				
				if($jenis_Bast ==='CUST'){
					$Qry_Comp	= "SELECT phone,latitude,longitude FROM customers WHERE id='".$nocust_Bast."'";
				}else{
					$Qry_Comp	= "SELECT phone,'' AS latitude,'' AS longitude FROM suppliers WHERE id='".$nocust_Bast."'";
				}
				...
				if($rows_detail){
					foreach($rows_detail as $keyD=>$valD){
						...
						$Qry_Trans		= "SELECT quotation_nomor,pono FROM trans_details WHERE id='".$Trans_Item."'";
						$det_Trans		= $this->db->query($Qry_Trans)->result();
						$Nomor_PO		= $det_Trans[0]->pono;
						$Nomor_Quot		= $det_Trans[0]->quotation_nomor;
