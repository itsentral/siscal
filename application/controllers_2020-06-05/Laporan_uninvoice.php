...
class Laporan_uninvoice extends CI_Controller {	
	...
	public function qry_letter($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		...
		if($like_value){
			...
			$WHERE	.="(
						det_so.no_so LIKE '%".$this->rental->escape_like_str($like_value)."%',
						OR det_so.tgl_so LIKE '%".$this->rental->escape_like_str($like_value)."%',
						OR det_so.customer_name LIKE '%".$this->rental->escape_like_str($like_value)."%',
						OR det_quot.nomor LIKE '%".$this->rental->escape_like_str($like_value)."%',
						OR det_quot.pono LIKE '%".$this->rental->escape_like_str($like_value)."%',
						OR det_quot.member_name LIKE '%".$this->rental->escape_like_str($like_value)."%'
						)";
		}
