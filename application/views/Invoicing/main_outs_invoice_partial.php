...
<form action="#" method="POST" id="form-proses" enctype="multipart/form-data">
	<div class="box box-warning">
		<div class="box-header">
			<h4 class="box-title"><i class="fa fa-check-square"></i> GENERATE INVOICE PARTIAL</h4>
...
<script type="text/javascript">
	...
	function view_order(kode){
		...
		$.ajax({
			url			: baseurl,
			type		: "POST",
			data		: {'kode_process':kode,'flag_so':'Y'},
			beforeSend: function() {
				$('#Loading_tes').show();
			},
	...
	function CreateInvoice(CodeSo){
		loading_spinner();
		let Link_Process		= base_url +'/'+ active_controller+'/invoicing_process?code_inv='+encodeURIComponent(CodeSo)+'&flag_order=Y';
	...
	$(document).on('click','#btn_process_inv',(e)=>{
		...
		loading_spinner();
		let Link_Process		= base_url +'/'+ active_controller+'/invoicing_process?code_inv='+encodeURIComponent(CodeTerpilih)+'&flag_order=Y';
		window.location.href	= Link_Process;
	});
