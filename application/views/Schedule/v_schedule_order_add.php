					...
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div>
								...
								<?php
									echo form_input(array('id'=>'letter_order_id','name'=>'letter_order_id','type'=>'hidden'),$rows_header->id);
									echo form_input(array('id'=>'kode_proses','name'=>'kode_proses','type'=>'hidden'),$kode_proses);
									echo form_input(array('id'=>'revisi','name'=>'revisi','type'=>'hidden'),'');
							...
							<tbody id="list_detail">
								...
								if($rows_detail){
									...
									foreach($rows_detail as $ketD=>$valD){
										...
										$Qty			= $valD->qty;
										$Qty_Schedule	= $valD->qty_schedule;
										$Range_Alat		= $valD->range;
										$Satuan			= $valD->piece_id;
										$Def_CodeSupp	= $valD->supplier_id;
										$Def_NameSupp	= $valD->supplier_name;
										$Tipe_Process	= $valD->tipe;
										...
										$insitu	='N';
										$kode_subcon	= $subcon;
										if(strtolower($Tipe_Process)=='insitu'){
											if($Def_CodeSupp !='COMP-001'){
												$kode_subcon	='Y';
											}
											$insitu	='Y';									
										}else if(strtolower($Tipe_Process) == 'labs'){
											$labs	='Y';
										}else if(strtolower($Tipe_Process) == 'subcon'){
											$subcon	='Y';
										}
										
										$Flag_Split		= 'N';
										
										$Qty_Outs		= $Qty - $Qty_Schedule;
										if($Qty_Outs > 0){											
											$intL++;
											echo'<tr id="tr_urut_'.$intL.'">';
												...
												echo form_input(array('id'=>'waktu_tempuh_'.$intL,'name'=>'detDetail['.$intL.'][waktu_tempuh]','type'=>'hidden'),0);
												echo form_input(array('id'=>'urut_id_'.$intL,'name'=>'detDetail['.$intL.'][urut_id]','type'=>'hidden', 'class'=>$Code_DetQuot),0);
												...
												<td class="text-center">';
													if($labs=='Y' || $subcon=='Y'){
														echo form_input(array('id'=>'delivery_date_'.$intL,'name'=>'detDetail['.$intL.'][delivery_date]','class'=>'form-control tanggal','readOnly'=>true));
													}else{
														echo '-';
													}
												echo'
												...
												<td class="text-center">';
													if($labs=='Y' || $subcon=='Y'){
														...
														$Query_Receive	= "SELECT rec_date FROM quotation_detail_receives WHERE quotation_detail_id = '".$Code_DetQuot."' AND letter_order_id = '".$rows_header->id."'";
														$rows_Receive	= $this->db->query($Query_Receive)->row();
														if($rows_Receive){
															$Plan_Pickup	= $rows_Receive->rec_date;
														}
														echo form_input(array('id'=>'pick_date_'.$intL,'name'=>'detDetail['.$intL.'][pick_date]','class'=>'form-control tanggal','readOnly'=>true),$Plan_Pickup);
													}else{
														echo '-';
													}
												echo'
												...
												<td class="text-center">';
													if($subcon=='Y' && $insitu=='N'){
														echo form_input(array('id'=>'subcon_pick_date_'.$intL,'name'=>'detDetail['.$intL.'][subcon_pick_date]','class'=>'form-control tanggal','readOnly'=>true));
													}else{
														echo '-';
													}
												echo'
												...
												<td class="text-center">';
													if($subcon=='Y'){
														echo form_input(array('id'=>'process_date_'.$intL,'name'=>'detDetail['.$intL.'][process_date]','class'=>'form-control tanggal','readOnly'=>true));
													}else{
														echo '-';
													}
												echo'
												...
												<td class="text-center">';
													if($subcon=='Y' && $insitu=='N'){
														echo form_input(array('id'=>'subcon_send_date_'.$intL,'name'=>'detDetail['.$intL.'][subcon_send_date]','class'=>'form-control tanggal','readOnly'=>true));
													}else{
														echo '-';
													}
												echo'
...
<script>
	...
	$(document).ready(function(){
		...
		$('#list_detail').find('tr').each(function(){
			...
			var kodes	= id_tr[2];
			var nil		= $('#qty_process_'+kodes).val();
	...
	$(document).on('click','#btn_batal_schedule', (e)=>{
		...
		let SalesOrder	= $('#letter_order_id').val();
		let CodeProcess	= $('#kode_proses').val();
		
		 $.post(base_url +'/'+ active_controller+'/CancelScheduleProcess',{'code':SalesOrder,'kode_proses':CodeProcess}, function(response) {
			$('#loader_proses_save').hide();
	...
	$(document).on('click','#btn_process_order', async(e)=>{
		...
		try{			
			...
			const ParamProcess	= {
				'action'		: 'save_create_schedule_order',
				'parameter'		: formData,
