...
if ($tipe == '1') {
	...
	$judul	= "Late Calibration Process Insitu";
}
				...
				if (isset($records) && $records) {
					...
					foreach ($records as $key => $val) {
						...
						if ($tipe == '1') {
							...
							$leadtime	= ((strtotime($sekarang) - strtotime($plan_ambil)) / (60 * 60 * 24)) - 2;
						} else if ($tipe == '8') {
							if ($val['insitu'] == 'Y') {
								$Kategori	= 'Insitu';
								$plan_ambil	= ($val['plan_process_date']);
