		...
		<table id="example1" class="table table-bordered table-striped">
			<thead>
				<tr class='bg-blue'>
					...
					<th class="text-center">Status SO</th>
			  ...
			  if($row){
					...
					foreach($row as $datas){
						...
						echo"<tr>";							
							...
							echo"<td align='left'>".$datas->tgl_so."</td>"; 
							echo"<td align='left'>".$datas->no_so."</td>"; 
							echo"<td align='left'>".$datas->customer_name."</td>"; 
							echo"<td align='left'>".$datas->sts_so."</td>";
							echo"<td align='left'>".$datas->reserved."</td>";
                            echo"<td align='left'>".$datas->flag_invoice."</td>";	
                            echo"<td align='left'>".$datas->flag_certificate."</td>";		
                            echo"<td align='left'>".$datas->flag_so_insitu."</td>";								
							echo"<td align='center'>";
								if($akses_menu['update']=='1'){
									echo"<button class='btn btn-sm btn-warning edit' title='Tampilkan Di Schedule' data-id='".$datas->id."'><i class='fa fa-edit'></i>";
								}									
...
<script>
	...
	$(document).on('click', '.edit', function(){
		...
		swal({
		  ...
		  text: "Data akan muncul di menu schedule !!!",
		  type: "warning",
		...
		function(isConfirm) {
			if (isConfirm) {
				loading_spinner();
				$.ajax({
					url			: base_url + active_controller+'/edit_schedule/'+id,
					type		: "POST",
					data		: "id="+id,
					cache		: false,
					dataType	: 'json',
					...
					success		: function(data){								
						if(data.status == 1){											
							swal({
							 ...
							 window.location.href = base_url + active_controller+'/schedule';
						}
