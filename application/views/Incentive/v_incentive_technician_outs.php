...
<script type="text/javascript">
	...
	$(document).on('click','#btn_download_outs',()=>{
		...
		let LinkDownload	= base_url +'/'+ active_controller+'/export_technician_incentive_outs?teknisi='+encodeURIComponent(ChosenTeknisi);
	...
	$(document).on('click','#btn_process_outs',(e)=>{
		...
		if(ChosenTeknisi == '' || ChosenTeknisi ==  null){
			swal({
			  title				: "Warning !",
			  text				: 'Empty Technician. Please choose technician first',						
			  type				: "warning"
			});				
			return false;	
		}
		...
		loading_spinner();
		let Link_Process		= base_url +'/'+ active_controller+'/technician_incentive_cpr_process';
