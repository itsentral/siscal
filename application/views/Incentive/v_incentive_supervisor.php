			...
			<table id="my-grid" class="table table-bordered table-striped">
				<thead>
					<tr class="bg-navy-blue">
						...
						<th class="text-center">Supervisor<br>(Penyelia)</th>
	...
	function data_display(){
		...
		let table_data 		= $('#my-grid').DataTable({
			...
			"ajax":{
				...
				type	: "post",
				data 	: {'teknisi':TechChosen,'bulan':MonthChosen,'tahun':YearChosen},
				cache	: false,
				beforeSend: function() {
					$('#Loading_tes').show();
				}, 
		...
		swal({
			...
			function(isConfirm) {					
				if (isConfirm) {
					...
					var baseurl		= base_url +'/'+ active_controller+'/save_payment_supervisor_cpr';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
		...
		swal({
			...
			function(isConfirm) {					
				if (isConfirm) {
					loading_spinner_new();
					var formData 	= new FormData($('#form_proses_preview')[0]);
					var baseurl		= base_url +'/'+ active_controller+'/save_cancel_supervisor_cpr';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
