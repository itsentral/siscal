	...
	<div class="box box-warning">
		...
		<div class="box-body">
			...
			<div class='row'>
				<div class="col-sm-6 col-xs-12">
					<div class="form-group">
						<label class="control-label"><b>Technician <span class='text-red'>*</span></b></label>
						<?php
				...
				<div class="col-sm-6 col-xs-12">
					<div class="form-group">
						...
						<?php
							echo form_textarea(array('id'=>'descr','name'=>'descr','class'=>'form-control input-sm','readOnly'=>true,'cols'=>75, 'rows'=>2),'Pembayaran Insentif Teknisi atas '.$rows_teknisi->nama);						
						?>
					</div>
				</div>				
			</div>
						...
						<tbody id="list_detail">
							...
							if($rows_detail){
								foreach($rows_detail as $key=>$row){
									...
									$Tgl_Letter		= $row['tgl_so'];
									$Tgl_Process	= $row['actual_process_date'];
									$Insitu_Status	= $row['insitu'];
									$Prosen_Inc	 	= 1;
									if($Tgl_Process > $Tgl_Compare  && $Insitu_Status === 'Y'){
										$Prosen_Inc	 	= 2;
									}
		...
		swal({
			...
			function(isConfirm) {					
				if (isConfirm) {
					...
					var baseurl		= base_url +'/'+ active_controller+'/save_process_technician_incentive';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
