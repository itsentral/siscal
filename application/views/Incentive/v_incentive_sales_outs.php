			...
			<table id="my-grid" class="table table-bordered table-striped">
				<thead>
					<tr class="bg-navy-blue">
						...
						<th class="text-center">Cust Fee</th>
...
<script type="text/javascript">
	...
	$(document).on('click','#btn_download_outs',()=>{
		let ChosenSales	= $('#sales').val();
		let LinkDownload	= base_url +'/'+ active_controller+'/export_sales_incentive_outs?sales='+encodeURIComponent(ChosenSales);
	...
	function data_display(){
		...
		let table_data 		= $('#my-grid').DataTable({
			...
			"columnDefs": [
				...
				{"targets":6,"sClass":"text-right","searchable":false,"orderable": false},
				{"targets":7,"sClass":"text-right","searchable":false,"orderable": false},
				{"targets":8,"sClass":"text-right","searchable":false,"orderable": false},
				{"targets":9,"sClass":"text-center","searchable":false,"orderable": false},
				{"targets":10,"sClass":"text-center","searchable":false,"orderable": false},
				{"targets":11,"sClass":"text-right","searchable":false,"orderable": false},
				{"targets":12,"sClass":"text-center","searchable":false,"orderable": false}
			],
	...
	$(document).on('click','#btn_process_outs',(e)=>{
		e.preventDefault();
		let ChosenSales	= $('#sales').val();
		if(ChosenSales == '' || ChosenSales ==  null){
			swal({
			  title				: "Warning !",
			  text				: 'Empty Salesman. Please choose Salesman first',						
			  type				: "warning"
			});				
			return false;	
		}
		...
		loading_spinner();
		let Link_Process		= base_url +'/'+ active_controller+'/sales_incentive_cpr_process';
