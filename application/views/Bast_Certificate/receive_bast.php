			...
			<div class='row'>
				<div class="col-sm-6">
					<div class="form-group">
						...
						<?php
							echo form_input(array('id'=>'receive_by','name'=>'receive_by','class'=>'form-control input-sm'));
				...
				<div class="col-sm-6">
					<div class="form-group">
						<label class="control-label">Tgl Terima Customer</label>
						<?php
							
							echo form_input(array('id'=>'receive_date','name'=>'receive_date','class'=>'form-control input-sm','readOnly'=>true),date('Y-m-d'));
						?>
					</div>
				</div>
...
<script>
	...
	$(document).on('click','#btn-save',function(e){
		...
		var pengirim	= $('#send_by').val();
		var penerima	= $('#receive_by').val();
		
		if(pengirim == '' || pengirim ==null || pengirim =='-'){
			swal({
			  title				: "Error Message !",
			  text				: 'Empty sender name, please input sender name...',						
			  type				: "warning"
			});
		...
		if(penerima == '' || penerima ==null || penerima =='-'){
			swal({
			  title				: "Error Message !",
			  text				: 'Empty receive by, please input receive by...',						
			  type				: "warning"
			});
			$('#btn-save, #btn-back').prop('disabled',false);
			return false;
		...
		swal({
			...
			function(isConfirm) {					
				if (isConfirm) {
					...
					var baseurl		= base_url +'index.php/'+ active_controller+'/update_bast';
					$.ajax({
						url			: baseurl,
						type		: "POST",
						data		: formData,
