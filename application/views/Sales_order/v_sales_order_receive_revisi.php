									...
									foreach($rows_detail as $ketD=>$valD){
										...
										echo'<tr id="tr_urut_'.$intL.'">';
											...
											<td class="text-center">
												<select name="detDetail['.$intL.'][get_tool]" id="get_tool_'.$intL.'" class="form-control chosen-select">';
													foreach($Arr_Type as $keyType=>$valType){
														$Yuup	= ($valType == $Get_Tool)?'selected':'';
														echo '
														<option value="'.$valType.'" '.$Yuup.'>'.$valType.'</option>
														';
													}
											echo'</select>
											</td>
...
<script>
	...
	const selectOrder = (id)=>{	
		...
		let Ket_Tipe	= '<span class="badge bg-maroon-active">SUBCON</span>';
		if(Tipe_Pros == 'L'){
			Ket_Tipe	= '<span class="badge bg-green-active">LABS</span>';
		}
		...
		if(ada==0){
			let Template	='<tr id="tr_urut_'+loop+'">'+
				...
				Template	+='</td>'+
							...
							'<td class="text-center">'+
								'<select name="detDetail['+loop+'][get_tool]" id="get_tool_'+loop+'" class="form-control chosen-select">'+
			...
			$('#qty_detail_'+loop).spinner({					
			 ...
			 $('#get_tool_'+loop).chosen({width: '100%'});
			 if(Code_Supp !='COMP-001'){
