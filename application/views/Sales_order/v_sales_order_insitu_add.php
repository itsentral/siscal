						...
						<table class="table table-striped table-bordered" id="my-grid">
							<thead>
								<tr class="bg-navy-active">
									...
									<th class="text-center">Insitu Area</th>
									...
									foreach($rows_detail as $ketD=>$valD){
										...
										if($Qty_Outs > 0){											
											...
											echo'<tr id="tr_urut_'.$intL.'">';
												...
												echo form_input(array('id'=>'tipe_'.$intL,'name'=>'detDetail['.$intL.'][tipe]','type'=>'hidden'),'I');
												echo'											
												...
												<td class="text-center">'.form_input(array('id'=>'qty_detail_'.$intL,'name'=>'detDetail['.$intL.'][qty]','class'=>'form-control','readOnly'=>true,'style'=>'max-width:50px !important;'),$Qty_Outs).'</td>
												<td class="text-left text-wrap">'.$Ket_Cust.'</td>
												<td class="text-center">'.form_textarea(array('id'=>'desc_'.$intL,'name'=>'detDetail['.$intL.'][descr]','class'=>'form-control text-up text-wrap','cols'=>50,'rows'=>2),$Keterangan).'</td>
												<td class="text-center">';
													...
													if($rows_delivery){
														foreach($rows_delivery as $keyDelv=>$valDelv){
															...
															echo '
															<option value="'.$Code_Unik.'">'.strtoupper($valDelv).'</option>
															';
														}
													}
													echo'
...
<script>
	...
	$(document).on('click','#btn_process_order', async(e)=>{
		...
		try{			
			...
			const ParamProcess	= {
				'action'		: 'save_create_letter_order',
				'parameter'		: formData,
