...
class Monitoring_tool_close extends CI_Controller { 
	...
	function get_data_display(){
		...
		$sql = "SELECT
					*,
					(@row:=@row+1) AS urut
				FROM
					view_monitor_close_tool_schedules,
				(SELECT @row:=0) r 
				WHERE ".$WHERE;
		//print_r($sql);exit();
	...
	function download_excel_tool(){
		...
		$Judul			="Report Monitoring Proses Kalibrasi (CLOSE)";
		$Title			="Monitor";	
		set_time_limit(0);
		...
		header('Content-Disposition: attachment;filename="Report_Monitoring_Tool_Close_'.date('YmdHis').'.xls"');
		//unduh file
