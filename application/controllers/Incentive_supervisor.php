...
class Incentive_supervisor extends CI_Controller { 
	...
	public function index(){
		...
		$data = array(
			'title'			=> 'MANAGE INCENTIVE SUPERVISOR',
			'action'		=> 'index',
			'akses_menu'	=> $Arr_Akses,
			'rows_teknisi'	=> $rows_Teknisi
		);
		history('View List Incentive Supervisor');
		$this->load->view($this->folder.'/v_incentive_supervisor',$data);
	}
	...
	*/
	function get_data_display(){
		...
		foreach($query->result_array() as $row)
		{
			...
			$Template		= '<button type="button" class="btn btn-sm bg-navy-active" onClick = "ActionIncentive({code:\''.$CPR_No.'\',action :\'view_supervisor_cpr\',title:\'VIEW DETAIL\'});" title="VIEW DETAIL"> <i class="fa fa-search"></i> </button>';
			
			if(($Arr_Akses['delete'] == '1' || $Arr_Akses['update'] == '1') && $Status == 'OPN'){
				$Template		.= '&nbsp;&nbsp;<button type="button" class="btn btn-sm bg-red" onClick = "ActionIncentive({code:\''.$CPR_No.'\',action :\'cancel_supervisor_cpr\',title:\'CANCEL INCENTIVE\'});" title="CANCEL INCENTIVE"> <i class="fa fa-trash-o"></i> </button>';
				
				$Template		.= '&nbsp;&nbsp;<button type="button" class="btn btn-sm btn-primary" onClick = "ActionIncentive({code:\''.$CPR_No.'\',action :\'payment_supervisor_cpr\',title:\'UPDATE PAYMENT\'});" title="UPDATE PAYMENT"> <i class="fa fa-money"></i> </button>';
			}
			if($Arr_Akses['download'] == '1' && $Status == 'OPN'){				
				$Template		.= "&nbsp;&nbsp;<a href='".site_url()."/Incentive_supervisor/print_supervisor_cpr?cpr=".$CPR_No."' title='Print CPR' class='btn btn-sm btn-success' target='_blank'> <i class='fa fa-print'></i> </a>";
	...
	*/
	...
	function list_outstanding_incentive(){		
		$rows_Teknisi		= $this->master_model->getArray('members',array('division_id'	=> 'DIV-002'),'id','nama');
		$data = array(
			'title'			=> 'OUTSTANDING SUPERVISOR INCENTIVE',
			'action'		=> 'list_outstanding_incentive',
			'akses_menu'	=> $this->Arr_Akses,
			'rows_teknisi'	=> $rows_Teknisi
		);
		history('View Oustanding Supervisor Incentive');
		$this->load->view($this->folder.'/v_incentive_supervisor_outs',$data);
	}
	...
	*/
	...
	function outstanding_incentive_cpr(){
		...
		if($Find_Tech){
			...
			$WHERE_Find .="supervisor_id = '".$Find_Tech."'";
		}
		...
		if($Like_Value){
			...
			$WHERE_Find	.="(
						...
						OR detail_invoice.supervisor_name LIKE '%".$this->db->escape_like_str($Like_Value)."%'
						)";
		}
		...
		$sql = "SELECT
					...
					detail_invoice.supervisor_id,
					detail_invoice.supervisor_name,
		...
		$columns_order_by = array(
			...
			4 => 'detail_invoice.supervisor_name',
		...
		foreach($query->result_array() as $row)
		{
			...
			$Code_SPV		= $row['supervisor_id'];
			$Name_SPV		= $row['supervisor_name'];
			$Qty_Tool		= $row['qty'];
			$HPP_Tool		= $row['hpp'];
			...
			$Code_Unik		= $Code_SPV.'^_^'.$Code_Tool.'^_^'.$Code_Letter;
	...
	function QueryProcess(){
		...
		$Query_process	= "SELECT
								...
								`det_trans`.`supervisor_id` AS `supervisor_id`,
								`det_trans`.`supervisor_name` AS `supervisor_name`,
								...
								`det_trans`.`flag_spv` AS `flag_spv`,
								x_order.invoice_no,
								x_order.grand_tot,
								x_order.pph23,
	...
	/*
	| ------------------------------------ |
	|		SUPERVISOR CPR PROCESS		   |
	| ------------------------------------ |
	*/
	function supervisor_incentive_cpr_process(){
		$Ok_Proses		= 0;
		...
		if($this->input->post()){
			...
			$WHERE			= "CONCAT_WS('^_^',detail_invoice.supervisor_id,detail_invoice.tool_id,detail_invoice.letter_order_id) IN('".$Imp_Code."')";
		...
		if($Ok_Proses == 1){
			$data = array(
				'title'			=> 'SUPERVISOR INCENTIVE CPR PROCESS',
				'action'		=> 'supervisor_incentive_cpr_process',
				'rows_detail'	=> $rows_Detail,
				'rows_teknisi'	=> $rows_Teknisi
			);
			
			$this->load->view($this->folder.'/v_incentive_supervisor_process',$data);
		}else{			
			$this->session->set_flashdata("alert_data", "<div class=\"alert alert-warning\" id=\"flash-message\">No record was found....</div>");
			redirect(site_url('Incentive_supervisor/'.$Link_Back));
			
		}
	}
	
	
	function save_process_supervisor_incentive(){
		...
		if($this->input->post()){
			...
			$CPR_Date		= date('Y-m-d');
			$CPR_Nomor		= 'CPR-SPV-'.date('Ym').'-'.date('dHis');
			...
			$Has_Ins_CPR_Head	= $this->db->insert('supervisor_incentives',$Ins_CPR_Head);
			...
			if($detDetail){
				...
				foreach($detDetail as $keyDetail=>$valDetail){
					...
					$Ins_CPR_Det	= array(
						'id'						=> $Code_Detail,
						'supervisor_incentive_id'	=> $CPR_Nomor,
					...
					$Has_Ins_CPR_Det	= $this->db->insert('supervisor_incentive_details',$Ins_CPR_Det);
					...
					$Query_Tool			= "SELECT
											...
											WHERE
												det_trans.flag_proses = 'Y'
											AND det_trans.supervisor_id = '".$Member_Code."'
											AND det_trans.flag_spv <> 'Y'
					...
					if($Arr_Imp_Code){
						...
						$Upd_Trans		= "UPDATE trans_data_details SET flag_spv = 'Y' WHERE id IN('".$Imp_Code."')";
			...
			if ($this->db->trans_status() !== TRUE || !empty($Pesan_Error)){
				...
				history('Create Supervisor Incentive CPR '.$CPR_Nomor);
			}
	...
	function view_supervisor_cpr(){
		$rows_Header		= $rows_Detail	= array();
		if($this->input->post()){
			$Code_CPR			= $this->input->post('code');
			$rows_Header		= $this->db->get_where('supervisor_incentives',array('id'=>$Code_CPR))->result();			
			$rows_Detail		= $this->db->get_where('supervisor_incentive_details',array('supervisor_incentive_id'=>$Code_CPR))->result();
		}
		$data = array(
			'title'			=> 'SUPERVISOR CPR DETAIL',
			'rows_header'	=> $rows_Header,
			'rows_detail'	=> $rows_Detail,
			'action'		=> 'view_supervisor_cpr',
			'category'		=> 'view'
		);
		$this->load->view($this->folder.'/v_incentive_supervisor_preview',$data); 
	}
	
	
	
	function print_supervisor_cpr($Code_CPR = ''){
		$rows_Header		= $rows_Detail	= array();
		if($this->input->get()){
			$Code_CPR			= urldecode($this->input->get('cpr'));
			$rows_Header		= $this->db->get_where('supervisor_incentives',array('id'=>$Code_CPR))->row();			
			$rows_Detail		= $this->db->get_where('supervisor_incentive_details',array('supervisor_incentive_id'=>$Code_CPR))->result();
		}
		
		$data 			= array(
			'title'			=> 'PRINT SUPERVISOR CPR',
			'action'		=> 'print_supervisor_cpr',
			'rows_header'	=> $rows_Header,
			'rows_detail'	=> $rows_Detail,
		...
		$this->load->view($this->folder.'/v_incentive_supervisor_print',$data); 
	}
	
	
	function payment_supervisor_cpr(){
		$rows_Header		= $rows_Detail	= array();
		if($this->input->post()){
			$Code_CPR			= $this->input->post('code');
			$rows_Header		= $this->db->get_where('supervisor_incentives',array('id'=>$Code_CPR))->result();			
	...
	function save_payment_supervisor_cpr(){
		$Arr_Return		= array(
			'status'		=> 2,
			'pesan'			=> 'No record was found....'
		);
		if($this->input->post()){
			...
			$rows_Check		= $this->db->get_where('supervisor_incentives',array('id'=>$Code_CPR))->row();
			...
			if($rows_Check->status !== 'OPN'){
				...
				$this->db->trans_begin();
				$this->db->update('supervisor_incentives',$Upd_header,array('id'=>$Code_CPR));
				...
				if ($this->db->trans_status() != TRUE){
					...
					history('Update Supervisor Incentive CPR '.$Code_CPR);
				}
			}
	...
	function cancel_supervisor_cpr(){
		$rows_Header		= $rows_Detail	= array();
		if($this->input->post()){
			$Code_CPR			= $this->input->post('code');
			$rows_Header		= $this->db->get_where('supervisor_incentives',array('id'=>$Code_CPR))->result();			
		...
		$data = array(
			'title'			=> 'SUPERVISOR CPR CANCELLATION',
			'rows_header'	=> $rows_Header,
			'rows_detail'	=> $rows_Detail,
			'action'		=> 'cancel_supervisor_cpr',
			'category'		=> 'cancel'
		);
		$this->load->view($this->folder.'/v_incentive_supervisor_preview',$data); 
	}
	
	function save_cancel_supervisor_cpr(){
		$Arr_Return		= array(
			'status'		=> 2,
			'pesan'			=> 'No record was found....'
		);
		...
		if($this->input->post()){
			...
			if($rows_Check->status !== 'OPN'){
				...
				$Has_Upd_CPR = $this->db->update('supervisor_incentives',$Upd_header,array('id'=>$Code_CPR));
				if($Has_Upd_CPR !== TRUE){
				...
				if($rows_Detail){
					...
					foreach($rows_Detail as $keyDetail=>$valDetail){
						...
						$Query_Tool		= "SELECT
											...
											WHERE
												det_trans.flag_proses = 'Y'
											AND det_trans.supervisor_id = '".$rows_Check->member_id."'
											AND det_trans.flag_spv = 'Y'
					...
					if($Arr_Imp_Code){
						$Imp_Code		= implode("','",$Arr_Imp_Code);
						$Upd_Trans		= "UPDATE trans_data_details SET flag_spv = 'N' WHERE id IN('".$Imp_Code."')";
				...
				if ($this->db->trans_status() != TRUE || !empty($Pesan_Error)){
					...
					history('Cancel Supervisor CPR  '.$Code_CPR);
				}
	...
	function export_supervisor_incentive_outs(){
		$Code_Teknisi	= '';
		if($this->input->get()){
			$Code_Teknisi	= urldecode($this->input->get('teknisi'));
		}
		$Judul			='Supervisor Incentive Report';
		$rows_Teknisi	= $this->master_model->getArray('members',array('division_id'	=> 'DIV-002'),'id','nama');
		$WHERE_Find		= "1=1";
		if($Code_Teknisi){
			if(!empty($WHERE_Find))$WHERE_Find .=" AND ";
			$WHERE_Find .="detail_invoice.supervisor_id = '".$Code_Teknisi."'";
		...
		$sheet->setCellValue('E'.$NewRow, 'Penyelias');
		...
		header('Content-Disposition: attachment;filename="Laporan_Outstanding_Penyelia_Insentif_'.date('YmdHis').'.xls"');
		//unduh file
